...
> **🚨 문제 상황: 탐험 요소 추가로 인한 전체 맵 스포너의 렌더링/연산 병목**
>
> - **기획의 한계 인지**: 초기에는 단순히 UI에서 포탈을 선택해 인스턴스 던전으로 진입하는 '방치형 게임' 구조로 기획. 그러나 게임 플레이의 재미(탐험의 몰입감)가 현저히 떨어진다고 판단하여, '로스트아크'처럼 하나의 큰 심리스 맵 곳곳에 몬스터가 스폰되는 필드 탐험 구조로 기획을 전면 수정.
> - **성능 저하 발생**: 기획 변경 후, 거대한 단일 맵에 배치된 수십 개의 `MonsterSpawner`가 동시에 몬스터를 스폰하고 AI(Behavior Tree)를 연산하게 됨.
> - **결과적 병목**: 플레이어의 시야에 보이지 않는 먼 곳의 몬스터들까지 렌더링 및 AI 틱을 소모하게 되어, 평균 144FPS가 120FPS 언저리로 떨어지는 **유의미한 프레임 드랍(Overhead)이 발생.**

**💡 해결 과정 (공간 분할 및 비동기 최적화)**

1. **공간 분할(Spatial Partitioning) 기반 Sector 시스템 도입**
- 맵 전체를 한 번에 연산하지 않고 여러 개의 구역(`Sector`)으로 분할.
- 각 `Sector`는 Trigger Collider를 통해 자신의 영역을 정의하고, 하위 오브젝트로 배치된 `MonsterSpawner`들을 리스트로 캐싱(`InitSpawners`).

2. **Manager를 통한 플레이어 위치 추적 및 선택적 활성화**
- 플레이어가 특정 구역에 진입(`OnTriggerEnter`)하면 `SectorManager`가 이를 감지하여 **현재 플레이어가 속한 Sector의 스포너만 활성화(`ActivateSector`)**하고, 이전 Sector의 몬스터들은 비활성화(`DeactivateSector`)하거나 메모리에서 해제하는 컬링(Culling) 구조 구축.

3. **안전한 비동기 리스폰(Respawn) 파이프라인 설계**
- 활성화된 구역 내에서 몬스터가 죽으면 일정 딜레이 후 다시 스폰되어야 함. 이를 `UniTask`와 `CancellationToken`으로 구현.
- 플레이어가 해당 구역을 갑자기 이탈하거나 씬을 전환할 때, 스폰 대기 중이던 비동기 Task가 런타임 에러(Missing Reference)를 뱉지 않도록 **`CancelAllSpawnTasks()`를 통해 토큰을 즉시 폐기(Dispose)하여 안전한 메모리 생명주기 관리** 구현.

**✅ 결과**
- "화면 밖의 연산은 생략한다"는 클라이언트 최적화의 대원칙을 적용하여, 심리스 필드 탐험이라는 기획적 목표를 달성하면서도 **다시 안정적인 144FPS 방어에 성공.**
- 방대한 JSON 스폰 데이터(`SpawnerData`)를 `DataManager`가 O(1)로 조회하여, 스포너가 켜지는 순간의 렉(Spike) 현상까지 함께 제거.
