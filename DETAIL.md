...
저에게 개발이란 단순히 기능을 동작시키는 일이 아닙니다.
**'내가 만든 결과물이 타인의 경험에 미칠 영향'** 을 고민하고,
**'아직 모르는 것을 직접 부딪혀 만들어가는 즐거움'** 을 찾는 과정입니다.

<br>

---

### 🤝 책임감 — 플레이어와 동료의 시간을 소중히 여기는 마음

- **플레이어에 대한 예의**: 버그나 프레임 드랍은 플레이어의 몰입을 방해하는
  '불친절'이라 생각합니다. 원인이 무엇이든 퍼포먼스가 흔들리는 순간
  타협하지 않고 끝까지 원인을 추적하는 것 — 그것이 저의 책임감입니다.

- **팀 프로젝트의 연결고리**: 게임 클라이언트 프로그래머는 팀 전체의 결과물이
  최종적으로 수렴되는 포지션입니다. 혼자 작성한 코드라도 타인이 읽고 수정할 것을
  염두에 두고 설계하는 것 — 그것이 협업이 시작되기 전부터 지킬 수 있는
  책임이라 믿습니다.
...
### 🚀 도전 — '만드는 즐거움'을 위해 기꺼이 뛰어들고, 미지를 퀘스트로 즐기는 태도

- **목적이 이끄는 선택의 가치**: 단순히 편한 길보다
  **'무엇을 배울 수 있는가'** 와 **'직접 만드는 즐거움'** 이 제 선택의 기준입니다.
  목표에 대한 확신이 있다면, 기술적 난제나 수고로움은
  성장을 위한 당연한 과정으로 즐깁니다.

- **불확실성 속에서 발견하는 몰입**: 낯선 기술이나 치명적인 버그 앞에서
  당황하기보다 본능적으로 원인을 파고듭니다.
  정답이 보이지 않는 순간일수록 오히려 더 깊이 몰입하게 되는 것이 저의 방식입니다.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; - 런타임 엑셀 파싱의 오버헤드를 에디터 타임 `ScriptableObject` 베이킹으로 이전하고 `DataManager`로 통합하여 **데이터 조회 속도 최적화 (`O(1)`)**
...
OperationKivotos(가제)는 4명의 캐릭터를 태그하여 전투를 진행하는 액션 RPG입니다. 단순히 기능을 구현하는 것에 그치지 않고, 실제 상용 게임 수준의 방대한 데이터를 효율적으로 관리하고 런타임 성능을 방어할 수 있는 아키텍처를 구축하는 데 집중한 프로젝트입니다.

**🔄 핵심 루프 (Core Loop)**
1. **탐험 & 전투** : 심리스 필드를 자유롭게 탐험하며 마주치는 적들을 처치
...
- **달성 성과** : `Addressables`와 `UniTask`를 결합하여 필요한 시점에 필요한 에셋만 로드하는 비동기 점진적 로딩 구조 구축. 실제 대규모 게임 환경에서도 안정적인 메모리 가용량을 확보할 수 있는 파이프라인 완성
...
- **인터페이스(`IDamageable`) 기반의 결합도 분리**
  - 피격 가능한 모든 객체는 `IDamageable`을 상속받아 `TakeDamage` 메서드를 각자의 방식대로 구현
