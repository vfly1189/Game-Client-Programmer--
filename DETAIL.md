...
- 기능이 동작하는 것보다, **왜 이렇게 설계했는지 설명할 수 있는 코드**를 더 중요하게 생각합니다.
  - [`IDamageable` 인터페이스로 공격자-피격자 결합 분리](#combat-bluearchive)
  - [UI 이벤트를 Observer 패턴으로 분리, 매 프레임 갱신 제거](#ui-optimization-bluearchive)

---

### 🚀 도전 — "재밌을 것 같으면 일단 뛰어드는 사람"
- 익숙한 방식이 동작하더라도, **더 나은 구조나 기술이 있다면 다시 짭니다.**
  - [단일 JSON → Excel/JSON 하이브리드 파이프라인으로 진화](#data-driven-trouble)
  - [C# Task 레이스 컨디션 → UniTask 메인스레드 동기화로 해결](#async-unitask-trouble)
- 완벽한 준비보다 **어느 정도 각이 잡히면 바로 실행하는 쪽**이 더 많이 배운다고 생각합니다.
  - [FSM의 유지보수 한계를 직접 겪고 Behavior Tree로 리팩토링](#fsm-to-bt)
